...
BOOL CScanner::RetrieveScannerParameters(READER_PARAMS* pReaderParams)
{
	...
	{
		LOG(WARNING) + L"Unable to retrieve Scanner Reader Parameters, is the Scanner enabled?";
	}
...
BOOL CScanner::SetScannerParameters(READER_PARAMS* pReaderParams, DWORD dwSize)
{
	...
	//  Flush any pending reads before applying the Scanner Parameters.  There is a critical section
	//  here as the read was being resubmitted after the flush before the reader parameters could be subsequently set.
	EnterCriticalSection(&m_scanner_prop_cs);
	DWORD dwFlushResult = lpfn_SCAN_Flush(m_hScanner);
...
BOOL CScanner::RetrieveScannerInterfaceParameters(INTERFACE_PARAMS* pInterfaceParams)
{
	...
	if (dwResult != E_SCN_SUCCESS)
	{
		LOG(WARNING) + L"Unable to retrieve Scanner Interface Parameters, is the Scanner enabled?";
	}
	return (dwResult == E_SCN_SUCCESS);
}
...
BOOL CScanner::SetScannerInterfaceParameters(INTERFACE_PARAMS* interfaceParams, DWORD dwSize)
{
	...
	if (dwResult != E_SCN_SUCCESS)
	{
		LOG(WARNING) + L"Unable to Set Scanner Reader Parameters";
	}
	return (dwResult == E_SCN_SUCCESS);
}
...
BOOL CScanner::RetrieveSupportedDecoders(WCHAR* szSupportedDecodersArray)
{
	...
	else
	{
		...
		LOG(WARNING) + L"Unable to retrieve supported decoders";
		bReturnValue = FALSE;
	}
...
BOOL CScanner::EnableDecoder(BYTE cDecoder)
{
	...
	if (IsDecoderSupported(cDecoder))
	{
		...
		if (dwResult == E_SCN_SUCCESS)
		{
			...
			if (dwResult != E_SCN_SUCCESS)
			{
				LOG(WARNING) + L"Unable to set Enabled Decoders";
			}
			else
				bRetVal = TRUE;
		}
...
BOOL CScanner::RetrieveDecoderParams(BYTE cDecoder, LPVOID lpvParams, DWORD dwSize)
{
	...
	if (dwResult != E_SCN_SUCCESS)
	{
		LOG(WARNING) + L"Unable to retrieve Scanner Decoder Parameters, is the Scanner enabled and the Decoder supported?";
		bRetVal = FALSE;
	}
	else
	{
...
BOOL CScanner::SetDecoderParams(BYTE cDecoder, LPVOID lpvParams, DWORD dwSize)
{
	...
	if ( dwResult != E_SCN_SUCCESS )
	{
		LOG(WARNING) + L"Unable to retrieve decoder parameters";
		bRetVal = FALSE;
	}
	else
	{
		...
		if (dwResult != E_SCN_SUCCESS)
		{
			LOG(WARNING) + L"Unable to set decoder parameters";
			bRetVal = FALSE;
		}
	}
	return bRetVal;
...
BOOL CScanner::SetDecoderLengths(BYTE cDecoder, int iMinLength, int iMaxLength)
{
	...
	if (dwResult != E_SCN_SUCCESS)
	{
		LOG(WARNING) + L"Unable to retrieve the current Decoder Parameters";
		bRetVal = FALSE;
	}
	else
	{
		...
		if (dwResult != E_SCN_SUCCESS)
		{
			LOG(WARNING) + L"Unable to Set current Decoder Parameters";
			bRetVal = FALSE;
		}
	}
	return bRetVal;
...
BOOL CScanner::RetrieveUPCEANParams(LPUPC_EAN_PARAMS lpParams)
{
	...
	if (dwResult != E_SCN_SUCCESS)
	{
		LOG(WARNING) + L"Unable to retrieve UPC EAN Parameters";
		bRetVal = FALSE;
	}
...
BOOL CScanner::SetUPCEANParams(LPUPC_EAN_PARAMS lpParams)
{
	...
	if (dwResult != E_SCN_SUCCESS)
	{
		LOG(WARNING) + L"Unable to set UPC EAN Parameters";
		bRetVal = FALSE;
	}
