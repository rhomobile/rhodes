...
public class Eventsource implements Runnable {
	...
	private static final RhoLogger LOG = RhoLogger.RHO_STRIP_LOG ? new RhoEmptyLogger() : 
		new RhoLogger("Eventsource");

	public static RubyModule EventsourceModule;    
	...
	public void run() {
		  LOG.INFO("$$$ Register Eventsource Ruby class");
   	  	  // register Ruby Module
          EventsourceModule = RubyAPI.defineModule("Eventsource");        
          // register Ruby method
          EventsourceModule.getSingletonClass().defineMethod("native_process_string", new RubyOneArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg0, RubyBlock block) {
				if ( arg0 instanceof RubyString )
				{
					...
					return ObjectFactory.createString(result);
					  ...
					  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Eventsource.native_process_string: wrong argument type.Should be String");
				}
			}
          });
          EventsourceModule.getSingletonClass().defineMethod("calc_summ", new RubyTwoArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg1, RubyValue arg2, RubyBlock block) {
				try {
						...
						if (arg1 instanceof RubyInteger) {
							...
							if (arg1 instanceof RubySymbol) {
								...
								x = rv.toInt();
								  ...
								  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Eventsource.calc_summ: wrong argument type.Should be Fixnum or Integer");
							}
						}
					}
					...
					e.printStackTrace();
					LOG.ERROR("Eventsource.calc_summ failed with exception", e);
