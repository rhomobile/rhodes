(function ($,rho,rhoUtil) {
  ...
  var loadAllSources = function(){
    try{
      console.log("loadAllSources called");
      var sources = Opal.Rho._scope.RhoConfig.$sources();
      console.log("got sources yay!!!" + JSON.stringify(sources));
      // for(var i=0;i<sources.length;i++){
      //   console.log("sources looping is name" + sources[i]);
      //   if(!sources[i]['loaded']){
      //     load_model(sources[i]['name'],false);
      //   }
      // }
      var v = "";
      sources.$each(function(attrib,value){
        console.log("attrib: " + attrib);
        console.log("value: " + JSON.stringify(value));
        console.log("laoded: " + value['loaded']);
        if(!value['loaded']){
          load_model(value['name'],false);
        }
      });
      //init_sources();
    }
  ...
  function dbConnection(source){
    ...
    if(partitions && partitions[source['partition']]){
      ...
      //console.log("Opal.Database is" + JSON.stringify(db))
  ...
  function initDbSource(db, source, db_partition, hash_migrate){
