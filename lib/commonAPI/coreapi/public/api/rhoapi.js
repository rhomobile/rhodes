var Rho = Rho || (function ($) {
    ...
    // ====================================================================================
    var createPropProxy_fallback = function (obj, propDescr, propAccess, apiReqFunc) {
        ...
        if (0 <= propAccess.indexOf('w')) {
            obj[accessorName('set', propName)] = propAccessReqFunc(apiReqFunc, propDescr, 'w');
        }
        if (0 <= propAccess.indexOf('r')) {
            obj[accessorName('get', propName)] = propAccessReqFunc(apiReqFunc, propDescr, 'r');
        }
    };
    ...
    if (propsSupport.js185) {
        ...
        createPropProxy = function (obj, propDescr, propAccess, apiReqFunc) {
            var propName = propDescr.split(':')[0];

            var propDef = {};
            if (0 <= propAccess.indexOf('r')) {
                propDef['get'] = propAccessReqFunc(apiReqFunc, propDescr, 'r');
            }
            if (0 <= propAccess.indexOf('w')) {
                propDef['set'] = propAccessReqFunc(apiReqFunc, propDescr, 'w');
            }
        ...
        createPropProxy = function (obj, propDescr, propAccess, apiReqFunc) {
            ...
            obj.__defineGetter__(propName, propAccessReqFunc(apiReqFunc, propDescr, 'r'));
            obj.__defineSetter__(propName, propAccessReqFunc(apiReqFunc, propDescr, 'w'));
        };
    } else {
