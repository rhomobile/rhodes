var Rho = Rho || (function ($) {
    ...
    var createEntityPropProxy_fallback = function (obj, EntityPropName, EntityPropAccess) {
    };

    var createEntityPropProxy = createEntityPropProxy_fallback;

    if (propsSupport.js185) {
        // the best case, js185 EntityProps are supported
        createEntityPropProxy = function (obj, EntityPropName, EntityPropAccess) {
            // TODO: implement enumeration of EntityPropProxy to extend target namespace with them.
            var js185EntityPropDef = {configurable: true, enumerable: false /* true */};
            if (0 <= EntityPropAccess.indexOf('r')) {
                js185EntityPropDef['get'] = function() { return this.__impl.get(EntityPropName); }
            }
            if (0 <= EntityPropAccess.indexOf('w')) {
                js185EntityPropDef['set'] = function(value) { this.__impl.set(EntityPropName, value)};
            }
            Object.defineProperty(obj, EntityPropName, js185EntityPropDef);
        };
    } else if (propsSupport.ffHackMethod) {
        // backup option, EntityProps are supported with firefox hack
        createEntityPropProxy = function (obj, EntityPropName, EntityPropAccess) {
            if (0 <= EntityPropAccess.indexOf('r')) {
                obj.__defineGetter__(EntityPropName, function() { return this.__impl.get(EntityPropName); });
            }
            if (0 <= EntityPropAccess.indexOf('w')) {
                obj.__defineSetter__(EntityPropName, function(value) { this.__impl.set(EntityPropName, value)});
            }
        };
    } else {
        // Sorry, no luck. We able provide just a default implementation with explicit accessors.
    ...
    function createEntityPropsProxy(obj, propDefs) {
        if (!(propDefs instanceof Array))
            throw 'Property definitions list should be Array instance';
        ...
        for (var i=0; i<propDefs.length; i++) {
            ...
            try {
                createEntityPropProxy(obj, propDef['propName'], propDef['propAccess']);
            } catch (ex) {
                // we unable to create property with this name, so log it
                incompatibleProps.push(name);
            }
    ...
    var util = {
        ...
        createEntityPropsProxy: createEntityPropsProxy,
