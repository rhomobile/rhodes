var Rho = Rho || (function ($) {
    ...
    var createEntityPropProxy_fallback = function (obj, Getter, Setter) {
    };
    ...
    if (propsSupport.js185) {
        ...
        createEntityPropProxy = function (obj, EntityPropName, Getter, Setter) {
            ...
            if (undefined != Getter && null != Getter) {
                js185EntityPropDef['get'] = Getter; 
            }
            if (undefined != Setter && null != Setter) {
                js185EntityPropDef['set'] = Setter;
            }
        ...
        createEntityPropProxy = function (obj, EntityPropName, EntityPropAccess) {
            if (ndefined != Getter && null != Getter) {
                obj.__defineGetter__(EntityPropName, Getter);
            }
            if (undefined != Setter && null != Setter) {
                obj.__defineSetter__(EntityPropName, Setter);
            }
        };
    } else {
        // Sorry, no luck. We able provide just a default implementation with explicit accessors.
    ...
    function createEntityPropsProxy(obj, propDefs) {
        ...
        for (var i=0; i<propDefs.length; i++) {
            ...
            try {
                createEntityPropProxy(obj, propDef['propName'], propDef['propGetter'], propDef['propSetter']);
            } catch (ex) {
                // we unable to create property with this name, so log it
                incompatibleProps.push(name);
            }
