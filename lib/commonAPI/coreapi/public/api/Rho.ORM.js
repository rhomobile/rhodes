...
(function ($, rho, rhoUtil) {
  ...
  function databaseFullResetEx(ary,reset_local_models){
  ...
  function databaseFullReset(reset_client_info,reset_local_models){
    ...
    if(Rho.RhoConnectClient.bulksyncState)
      Rho.RhoConnectClient.bulksyncState='0';

    $.each(Rhom.ORMHelper.getDbPartitions,function(name,db){
      if(src_partition == 'local' && !reset_local_models){
        //do nothing
      }else{
  ...
  function databaseFullResetAndLogout(){
    database_full_reset(true,true);
    if(Rho.RhoConnectClient !== undefined)
      Rho.RhoConnectClient.logout();
  }

  function databaseFullclientResetAndLogout(){
    database_full_reset(false,true);
    if(Rho.RhoConnectClient !== undefined)
      Rho.RhoConnectClient.logout();
  }
  ...
  var makeModel = function(modelName, builder) {
    ...
    return {
        ...
        deleteAll: function(args) {
          opalModel.$delete_all(__hash2(args));
        }
    };
  };
  ...
  var ORM = {
    ...
    databaseExport: function(partition){
      //TODO zip up db based on partition passed
    },
    databaseImport: function(partition,db){
      //TODO create db from db and partition
    },
    databaseLocalReset: function(partition){
      //TODO reset only non sync models
    },
    ...
    databaseFullResetAndLogout: function(){
      //TODO delete all from property bag(sources)
    },
    databaseFullReset: function(resetClientInfo,resetLocalModels){
      //TODO delete all from property bag and client_info, will clean sync or non sync depending on params
      databaseFullReset(resetClientInfo,resetLocalModels);
    },

    //look at ruby db reset functions (3 of them)
    ...
    haveLocalChanges: function(){
      //TODO returns true if any model object has db changes, change_values table
      //Get all models and loop through calling $any_model_changed?, return true if any found
