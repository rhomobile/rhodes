...
void Init_RubyAPI_Process(void)
{
	...
	rb_mProcess = rb_define_class_under(rb_mParent, "Process", rb_cObject);

    //Constructor should be not available
	//rb_define_alloc_func(rb_cBarcode1, rb_barcode1_allocate);
    //rb_undef_alloc_func(rb_mProcess);

    rb_define_singleton_method(rb_mProcess, "runApplication", rb_s_Process_runApplication, -1);
    ...
    rb_define_alias(rb_singleton_class(rb_mProcess), "run_application", "runApplication");
    rb_define_alias(rb_mProcess, "wait_for_application", "waitForApplication");
    
    rb_define_alias(rb_mProcess, "close_handle", "closeHandle");
    
    rb_define_alias(rb_mProcess, "get_process_exit_code", "getProcessExitCode");
