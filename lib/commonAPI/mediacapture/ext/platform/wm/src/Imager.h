...
#include "logging/RhoLog.h"		// Rhodes Logging
#include "common/RhoConf.h"		// Rhodes Configuration
...
// SPS - added from original CORE code
 ...
 */
	...
	// SPS - modified to use new callback mechanism
	WCHAR*	m_szImagerCaptureURI;	///<  Callback Event URI for ImagerCaptureEvent
	rho::apiGenerator::CMethodResult*	m_pTakePictureEvent;			///<  PictureEvent
	//PBModule*	m_pModule;	///<  Pointer to the CImagerModule object owning this CImager
	BOOL m_bSaveAsFile;	// Specifies when captured data file shoule be returned as data URI
	// SPS
	...
	*/
	...
	int SetImagerCaptureEvent(rho::apiGenerator::CMethodResult* eventCallback);
	// SPS
	...
	*  \return True
	*/
	int InvokeImagerCaptureEvent(BYTE* bData, int iLength);
...
//	HANDLE m_hImgStopEvent;
    ...
    BOOL SetCaptureSound(const char* cstr);
    const char* GetCaptureSound();
    void ApplicationFocusChange(bool bActivate);
 ...
 */
	...
	CImager(HINSTANCE hInstance,HWND hParent,int iTab);
	// SPS
/**
 ...
 */
	...
	const char* SetImageFilename(const char* cstr);
	const char* GetImageFilename(void);

	TCHAR m_lpSzFiletype[MAXURL + 1]; ///<Pointer to the type of the image 
