...
+ (const char*)responseHttpInfo:(NSHTTPURLResponse*)res {
    return [[NSString stringWithFormat:@"<NSHTTPURLResponse:<%p>, URL:[ %@ ], Headers:[ %@ ]", res, [res URL], [[res allHeaderFields] description]] UTF8String];
}
    ...
    if ( resp != nil )
    {
      ...
      if ( ((self.httpStatusCode==301)||(self.httpStatusCode==302)) && ( [self.httpHeaders objectForKey:@"location"] != nil ) ) {
        ...
        if ( url != nil )
        {
            ...
            if (!self.isStopped) {
                ...
                if (is_net_trace()) {
                  RAWTRACE2("$NetWKRequestProcess$ CRhoWKURLProtocol %s :: return redirect { %s }", [self selfIDstring], [CRhoWKURLProtocol responseHttpInfo:respo]);
                }
                ...
                [obj performSelector:selector withObject:resp withObject:redirReq];
      ...
      else
      {
        if (is_net_trace()) {
          ...
          if (!self.isStopped) {
              if (is_net_trace()) {
                  RAWTRACE2("$NetWKRequestProcess$ CRhoWKURLProtocol %s :: startLoadingInThread RESPONSE: { %s }", [self selfIDstring], [CRhoWKURLProtocol responseHttpInfo:resp]);
              }
              [mUrlTask didReceiveResponse:resp];
          }
          if (self.httpBody != nil) {
              if (!self.isStopped) {
                  [mUrlTask didReceiveData:self.httpBody];
              }
          }
          if (!self.isStopped) {
              [mUrlTask didFinish];
...
- (void)startLoading
{
    ...
    if (![CRhoWKURLProtocol canInitWithRequest:[self request]]) {
        ...
        // check for error
        if (_error != nil) {
            [mUrlTask didFailWithError:_error];
            return;
        }
        ...
        if (((_statusCode == 301) || (_statusCode == 302) || (_statusCode == 303) || (_statusCode == 307) || (_statusCode == 308)) && (_httpResponse != nil) ) {
            ...
            if (loc != nil) {
                ...
                NSObject* obj = (NSObject*)mUrlTask;
                [obj performSelector:selector withObject:_httpResponse withObject:_redirReq];
        ...
        // other answer
        [mUrlTask didReceiveResponse:_response];
        [mUrlTask didReceiveData:_data];
        [mUrlTask didFinish];
...
- (void)sendResponseWithResponseCode:(NSInteger)statusCode data:(NSData*)data
{
    ...
    if (!self.isStopped) {
        if (is_net_trace()) {
            RAWTRACE2("$NetWKRequestProcess$ CRhoWKURLProtocol %s :: sendResponseWithResponseCode RESPONSE: { %s }", [self selfIDstring], [CRhoWKURLProtocol responseHttpInfo:response]);
        }
        [mUrlTask didReceiveResponse:response];
    }
    if (data != nil) {
