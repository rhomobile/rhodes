...
IMPLEMENT_LOGCLASS(CWKBrowserEngine,"WKBrowser");
...
HWND              CWKBrowserEngine::g_hwndTabHTMLContainer = NULL;
CWKBrowserEngine* CWKBrowserEngine::g_hInstance = NULL;

//////////////////////////////////////////////////////////////////////////

CWKBrowserEngine* CWKBrowserEngine::getInstance()
{
    return g_hInstance;
}

CWKBrowserEngine* CWKBrowserEngine::getInstance(HWND hParentWnd, HINSTANCE hInstance)
{
    if (!g_hInstance)
    {
        g_hInstance = new CWKBrowserEngine(hParentWnd, hInstance);
    }
...
CWKBrowserEngine::CWKBrowserEngine(HWND hParentWnd, HINSTANCE hInstance) :
        m_parentHWND(NULL),
        m_hparentInst(NULL),
...
CWKBrowserEngine::~CWKBrowserEngine()
{
...
{		
    ...
    m_hwndTabHTML = CreateWindow(kMainWindowClassName, NULL, 
...
BOOL CWKBrowserEngine::Navigate(LPCTSTR tcURL, int iTabID)
{
...
BOOL CWKBrowserEngine::ResizeOnTab(int iInstID,RECT rcNewSize)
{    
...
BOOL CWKBrowserEngine::BackOnTab(int iInstID,int iPagesBack /*= 1*/)
{
    return Navigate(L"history:back", 0);
}

BOOL CWKBrowserEngine::ForwardOnTab(int iInstID)
{
    int iPagesForward = 1;
...
BOOL CWKBrowserEngine::ReloadOnTab(bool bFromCache, UINT iTab)
{
...
BOOL CWKBrowserEngine::StopOnTab(UINT iTab)
{
    return SendMessage(m_hwndTabHTML, DTM_STOP, 0, 0);
}

BOOL CWKBrowserEngine::ZoomPageOnTab(float fZoom, UINT iTab)
{
    return SendMessage(m_hwndTabHTML, DTM_ZOOMLEVEL, 0, (LPARAM)(DWORD)m_textZoomValue);
}

BOOL CWKBrowserEngine::GetTitleOnTab(LPTSTR szURL, UINT iMaxLen, UINT iTab)
{
    return FALSE;
}

void CWKBrowserEngine::RunMessageLoop(CMainWindow& mainWnd)
{
	MSG msg;
...
void CWKBrowserEngine::executeJavascript(const wchar_t* szJSFunction, int index)
{
...
void CWKBrowserEngine::OnDocumentComplete(LPCTSTR url)
{
...
void CWKBrowserEngine::setNavigationTimeout(unsigned int dwMilliseconds)
{
    m_dwNavigationTimeout = dwMilliseconds;
}

HWND CWKBrowserEngine::GetHTMLWND(int /*iTabID*/)
{
    return m_hwndTabHTML;
}

LRESULT CWKBrowserEngine::OnWebKitMessages(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    //TODO TAU
	/*bHandled = TRUE;
...
void CWKBrowserEngine::InvokeEngineEventMetaTag(LPTSTR tcHttpEquiv, LPTSTR tcContent)
{
...
void CWKBrowserEngine::InvokeEngineEventLoad(LPTSTR tcURL, EngineEventID eeEventID)
{
	...
	switch (eeEventID)
	{
		case EEID_BEFORENAVIGATE:
			...
			CloseHandle (CreateThread(NULL, 0, 
									&CWKBrowserEngine::NavigationTimeoutThread, 
									(LPVOID)this, 0, NULL));
...
void CWKBrowserEngine::InvokeEngineEventTitleChange(LPTSTR tcTitle)
{
...
* \date		October 2009
*/
LRESULT CWKBrowserEngine::WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg) 
	{
		...
		case WM_NOTIFY:
		{
            ...
            CWKBrowserEngine* mobileTab = CWKBrowserEngine::getInstance();
...
DWORD WINAPI CWKBrowserEngine::NavigationTimeoutThread( LPVOID lpParameter )
{
    CWKBrowserEngine* pIEEng = reinterpret_cast<CWKBrowserEngine*>(lpParameter);
    DWORD dwWaitResult;
...
BOOL CWKBrowserEngine::AddNewUrl(LPCTSTR urlNew)
{
...
void CWKBrowserEngine::DeleteCascade(CHistoryElement* fromThisElementOn)
{
...
LRESULT  CWKBrowserEngine::GetPreviousUrl(LPTSTR tcURL)
{
...
UINT CWKBrowserEngine::BackListSize()
{
...
int CWKBrowserEngine::GetTextZoomOnTab(UINT iTab)
{
	return m_textZoomValue;
}
BOOL CWKBrowserEngine::ZoomTextOnTab(int nZoom, UINT iTab)
{
...
DWORD WINAPI CWKBrowserEngine::RegisterWndProcThread(LPVOID lpParameter)
{
    //  We are passed a pointer to the engine we are interested in.
    CWKBrowserEngine* pEngine = (CWKBrowserEngine*)lpParameter;
