...
function get_destination_dirs(es, name) {
    for (var i in es) {
        p(es[i].section+"=0,\"Application"+"\\" + name + "\\"+es[i].destination+"\"");
    }
}
...
function pinf(platform,es,exts,name,vendor,srcdir,show_shortcut,is_icon,webkit,
    ...
    p("AddReg=RegKeys");
    p("CopyFiles=CopyToInstallDir,CopyPersistent"+((!usereruntime) && webkit ? ",CopyWebKitBin,CopyNPAPI,CopyConfig,CopySystemFiles" : "")
        +((!usereruntime) && (!webkit) && include_motocaps? ",CopyConfig" : "")+(show_shortcut && usereruntime ? ",Shortcuts" : "")+get_copyfiles_sections(es));
    p("");
    p("[SourceDisksNames]");
    p("1=,\"\",,\"" + srcdir + "\"");
    if (usereruntime) {
    ...
    if ((!usereruntime) && webkit) {
        p("CopyWebKitBin=0,\"Application\\" + name + "\"");
        p("CopyNPAPI=0,\"Application\\" + name + "\\NPAPI\"");
        p("CopyConfig=0,\"Application\\" + name + "\\Config\"");
    }
    if ((!usereruntime) && (!webkit) && include_motocaps) {
        p("CopyConfig=0,\"Application\\" + name + "\\Config\"");
    }
        
    get_destination_dirs(es, name);
    p("");
    p("[CopyPersistent]");
