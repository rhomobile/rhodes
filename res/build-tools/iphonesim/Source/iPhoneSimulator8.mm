...
#import "../XCode8/DVTFoundation.h"
#import "../XCode8/CoreSimulator.h"
#endif  // IOSSIM_USE_XCODE_6
...
@class DVTiPhoneSimulatorLocalMessenger;
...
@end
#import "../XCode8/DVTiPhoneSimulatorRemoteClient.h"
...
// (crbug.com/385030).
  ...
  // simulator's data directory.
    ...
    //MOHUS
//if ([session.sessionConfig.simulatedSystemRoot.sdkVersion isEqual:@"9.0"]) {
...
void PrintUsageOriginal() {
  fprintf(stderr, "Usage: iphonesim_6 [-d device] [-s sdkVersion] [-u homeDir] "
...
void PrintUsage() {
    fprintf(stderr, "Usage: iphonesim_6 <options> <command> ...\n");
...
extern "C" int iossim_main(int argc, char* const argv[]) {
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    ...
    if (strcmp(argv[1], "showsdks") == 0) {
        ...
        appPath = [NSString stringWithUTF8String: argv[2]];
        appName = [appPath lastPathComponent];
        
        if (argc > 3) {
            sdkVersion = [NSString stringWithCString:argv[3] encoding:NSUTF8StringEncoding];
        }
    ...
    //MOHUS
    /*
    Class cDVTFilePath = FindClassByName(@"DVTFilePath");
    DVTFilePath* ap = [cDVTFilePath filePathForPathString:appPath];
    Class cDVTSimulatorApplication = FindClassByName(@"DVTSimulatorApplication");
    DVTSimulatorApplication* sa = [cDVTSimulatorApplication _defaultSimulator];//[[cDVTSimulatorApplication _defaultSimulator] initWithPath:ap];
    
    //NSStringFromClass()
    
    for (id root in [cDVTSimulatorApplication allSimulatorApplications]) {
        DVTSimulatorApplication* ttt = (DVTSimulatorApplication*)root;
        printf("  '%s'\n", [[ttt bundleIdentifier] UTF8String]);
    }
    printf("  '%s'\n", [[sa bundleIdentifier] UTF8String]);
    //NSError* er =
    //[sa launchWithError:nil];
    */
...
extern "C" int iossim_main_original(int argc, char* const argv[]) {
  NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
