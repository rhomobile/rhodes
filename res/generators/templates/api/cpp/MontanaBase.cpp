...
const char I<%= $cur_module.name %>::<%= module_constant.name %>[] = "<%= module_constant.value %>"; <%
end; end %>
...
%>
    m_mapPropAccessors["<%= module_method.linked_property.name %>"] = new rho::apiGenerator::CMethodAccessor< I<%= $cur_module.name %> >( &I<%= $cur_module.name %>::<%= module_method.native_name%> ); <%
    $cur_module.getPropAliases(module_method.linked_property.name).each do |alias_name| %>
    m_mapPropAccessors["<%=alias_name %>"] = new rho::apiGenerator::CMethodAccessor< I<%= $cur_module.name %> >( &I<%= $cur_module.name %>::<%= module_method.native_name%> ); <%
end; else %>
...
void C<%= $cur_module.name %>Base::getProperties( const rho::Vector<::rho::String>& arrayofNames, CMethodResult& oResult)
{
...
void C<%= $cur_module.name %>Base::setProperty( const rho::String& propertyName,  const rho::String& propertyValue, CMethodResult& oResult)
{
    CMethodAccessor< I<%= $cur_module.name %> >* pAccessor = m_mapPropAccessors[propertyName];
...
void C<%= $cur_module.name %>Base::setProperties( const rho::Hashtable<::rho::String, rho::String>& propertyMap, CMethodResult& oResult)
{
