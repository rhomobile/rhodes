      ...
      xml_api_root.elements.each("MODULE") do |xml_module_item|
        ...
        module_item.hash_key_names = hash_objs
         ...
         xml_module_item.elements.each("PROPERTIES/PROPERTY") do |xml_module_property|
            ...
            if unsupporded_names.include?( module_property.name.upcase )
              raise "Property have errorneous name ! Module[#{module_item.name}].property[#{module_property.name}] is in the list of forbidden names #{unsupporded_names.join("','")}"
            end
