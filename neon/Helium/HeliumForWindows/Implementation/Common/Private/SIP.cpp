...
// Date:		2/3/2005
////////////////////////////////////////////////////////////////////////
void CSIP::HideSIP(HWND hwndHTML, bool bDoShowSip)
{
	//  Removing references to AYGShell (tested and works on WM and CE)
//	if(bDoShowSip)
//		SHSipPreference(hwndHTML, SIP_FORCEDOWN);
		//SipShowIM(SIPF_OFF);

	m_bAllowSIP = false;
	if (m_bSipControl) {
		MoveWindow(hSipWnd, m_rcPosition.left, m_rcPosition.top, -300, -300, TRUE);
	}
	m_bSIPOn = false;

	//CSpbLog::Log(LOW,SPB_LOG_INFORMATION, L"Hidden SIP", L"CSIP", DEVICE_ID, __LINE__); 
}
...
*  \date November 2009 (Converted from static to non-static.)
*/
BOOL CSIP::SetSipControl(HWND hwndHTML, SIP_CONTROL_VALUES bSet)
{
	m_bSipControl = (bSet == SIP_CONTROL_MANUAL);
	m_bAllowSIP = !(bSet == SIP_CONTROL_MANUAL);

	if (hSipWnd == INVALID_HANDLE_VALUE) 
		return FALSE;	
	
	GetWindowRect(hSipWnd, &m_rcPosition);
	if (bSet == SIP_CONTROL_MANUAL) 
	{
		//  Manual SIP Control
		MoveWindow(hSipWnd, m_rcPosition.left, m_rcPosition.top, -300, 0, TRUE);
	}
	else 
	{
		//  Automatic SIP Control
		MoveWindow(hSipWnd, m_rcPosition.left, m_rcPosition.top, 
			m_rcPosition.right-m_rcPosition.left, m_rcPosition.bottom-m_rcPosition.top, TRUE);
		//  Show and Hide the SIP to get Automatic to work
		ToggleSIP();
		ToggleSIP();
		//SHSipPreference(hwndHTML, SIP_UP);
	}
