...
{
	if(m_hWaveIn){
		m_pModule->Log(PB_LOG_WARNING, L"Name cannot be changed during Wave capture",
...
int CAudioCapture::Start()
{
	...
	if (m_hWaveIn == NULL)
    {
		...
		m_pModule->Log(PB_LOG_INFO, L"WAV: Starting the Wave capture",	_T(__FUNCTION__), __LINE__);
		...
		{			
			...
			{
				m_pModule->Log(PB_LOG_WARNING, L"Unable to allocate memory for audio capture buffers",	_T(__FUNCTION__), __LINE__);
		...
		else 
		{
			...
			{
				m_pModule->Log(PB_LOG_WARNING, L"WAV: Wavein: MMSYSERR_ALLOCATED",	_T(__FUNCTION__), __LINE__);
				
			}
			else if(res == MMSYSERR_NODRIVER)
			{
				m_pModule->Log(PB_LOG_WARNING, L"WAV: Wavein: MMSYSERR_NODRIVER",	_T(__FUNCTION__), __LINE__);
				
			}
			else if(res == MMSYSERR_BADDEVICEID)
			{
				m_pModule->Log(PB_LOG_WARNING, L"WAV: Wavein: MMSYSERR_BADDEVICEID",	_T(__FUNCTION__), __LINE__);
				
			}
			else if(res == MMSYSERR_NOMEM)
			{
				m_pModule->Log(PB_LOG_WARNING, L"WAV: Wavein: MMSYSERR_NOMEM",	_T(__FUNCTION__), __LINE__);
				
			}
			else if(res == WAVERR_BADFORMAT)
			{
				m_pModule->Log(PB_LOG_WARNING, L"WAV: Waven WAVERR_BADFORMAT",	_T(__FUNCTION__), __LINE__);
				
			}
			m_pModule->Log(PB_LOG_WARNING, L"WAV: Wavin failure",	_T(__FUNCTION__), __LINE__);
			
		}	
	}
	else 
	{
		m_pModule->Log(PB_LOG_INFO, L"WAV: Restarting WaveIn capture",	_T(__FUNCTION__), __LINE__);
		...
		m_pModule->Log(PB_LOG_INFO, L"WAV: WaveIn capture restarted",	_T(__FUNCTION__), __LINE__);
...
//			RETAILMSG(1, (L"WAV: WaveIn Data.. time %u \n", GetTickCount()));
		...
		{
			pObj->m_pModule->Log(PB_LOG_INFO, L"WAV: WaveIn device open",	_T(__FUNCTION__), __LINE__);
		}
		else if(uMsg == WIM_CLOSE)
		{
			pObj->m_pModule->Log(PB_LOG_INFO, L"WAV: WaveIn device closed",	_T(__FUNCTION__), __LINE__);
...
DWORD WINAPI CAudioCapture::threadProc(LPVOID lpParam)
{
	...
	if (pAudioCapture)
	{
		if(pAudioCapture->eventHandler()==AC_SAVED){
			if(pAudioCapture->m_pModule->SendAudioFile(pAudioCapture->m_lpzFilename))
				return 0;
			pAudioCapture->m_pModule->Log(PB_LOG_WARNING, L"WAV: Could not transfer the file",	_T(__FUNCTION__), __LINE__);
		}
		return 1;
	}
	return -1;
...
AC_RETURN CAudioCapture::eventHandler()
{
	...
	if (m_hWaveIn)
	{
		m_pModule->Log(PB_LOG_INFO, L"WAV: WaveIn capture stopping",	_T(__FUNCTION__), __LINE__);
		waveInStop(m_hWaveIn);

		m_pModule->Log(PB_LOG_INFO, L"WAV: WaveIn capture resetting",	_T(__FUNCTION__), __LINE__);
		waveInReset(m_hWaveIn);

		m_pModule->Log(PB_LOG_INFO, L"WAV: WaveIn capture unpreparing headers",	_T(__FUNCTION__), __LINE__);
		
		waveInUnprepareHeader(m_hWaveIn, &m_waveHdrA, sizeof(WAVEHDR));
		waveInUnprepareHeader(m_hWaveIn, &m_waveHdrB, sizeof(WAVEHDR));

		waveInClose(m_hWaveIn);
		m_pModule->Log(PB_LOG_INFO, L"WAV: Audio recording stopped successfully",	_T(__FUNCTION__), __LINE__);
...
void CAudioCapture::processRawFile()
{
	...
	if (m_hFile == INVALID_HANDLE_VALUE)
	{
		...
		if (pCopyBuf == NULL) {
			m_pModule->Log(PB_LOG_WARNING, L"Unable to get enough memory for copy operation",	_T(__FUNCTION__), __LINE__);
			return;
		}
		...
		if( hWavFile == INVALID_HANDLE_VALUE ) {
			m_pModule->Log(PB_LOG_WARNING, L"Unable to open file",	_T(__FUNCTION__), __LINE__);
		...
		if( m_hFile == INVALID_HANDLE_VALUE ) {
			m_pModule->Log(PB_LOG_WARNING, L"Unable to open Raw file",	_T(__FUNCTION__), __LINE__);
		...
		if (! WriteFile(hWavFile, &FileHeader, sizeof(FileHeader), &dwBytesWritten, NULL)) {
			m_pModule->Log(PB_LOG_WARNING, L"Error writing file header",	_T(__FUNCTION__), __LINE__);
			goto ERROR_EXIT;
		}

		// write the wave header
		if (! WriteFile(hWavFile, &WaveHeader, sizeof(WaveHeader), &dwBytesWritten, NULL)) {
			m_pModule->Log(PB_LOG_WARNING, L"Error writing wave header",	_T(__FUNCTION__), __LINE__);
			goto ERROR_EXIT;
		}

		// write the wave format
		if (! WriteFile(hWavFile, &m_wfx, WaveHeader.dwSize, &dwBytesWritten, NULL)) {
			m_pModule->Log(PB_LOG_WARNING, L"Error writing wave format",	_T(__FUNCTION__), __LINE__);
			goto ERROR_EXIT;
		}

		// write the data header
		if (! WriteFile(hWavFile, &DataHeader, sizeof(DataHeader), &dwBytesWritten, NULL)) {
			m_pModule->Log(PB_LOG_WARNING, L"Error writing PCM data header",	_T(__FUNCTION__), __LINE__);
			goto ERROR_EXIT;
		}
		...
		do {
			...
			if (! ReadFile(m_hFile, pCopyBuf, dwChunkSize, &dwBytesRead, NULL) || dwBytesRead < dwChunkSize) {
				m_pModule->Log(PB_LOG_WARNING, L"Error reading raw file",	_T(__FUNCTION__), __LINE__);
				goto ERROR_EXIT;
			}

			// write the PCM data
			if (! WriteFile(hWavFile, pCopyBuf, dwBytesRead, &dwBytesWritten, NULL)) {
				m_pModule->Log(PB_LOG_WARNING, L"Error writing PCM data",	_T(__FUNCTION__), __LINE__);
				goto ERROR_EXIT;
			}
