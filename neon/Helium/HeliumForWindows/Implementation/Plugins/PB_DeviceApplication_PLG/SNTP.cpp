...
CSNTPClient::CSNTPClient(PBModule* pModule)
{
	m_dwTimeout = 5000; //Default timeout of 5 seconds
	m_pModule = pModule;
}

BOOL CSNTPClient::GetServerTime(char* pszHostName, NtpServerResponse& response, int nPort, BOOL modem)
{
	TCHAR cLog[256];
...
void CSNTPClient::SetPBModule(PBModule* pModule)
{
	m_pModule = pModule;
}
