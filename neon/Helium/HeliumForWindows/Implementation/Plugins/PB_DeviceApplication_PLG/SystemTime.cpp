...
CNtpSocket::CNtpSocket(PBModule* pModule)
{
	m_pModule = pModule;
...
{
	if (m_pModule != NULL)
		m_pModule->Log(PB_LOG_INFO, L"Close Socket", _T(__FUNCTION__), __LINE__); 
	Close();
}
...
BOOL CNtpSocket::Create()
{
	...
	if (m_pModule != NULL)
		m_pModule->Log(PB_LOG_INFO,  L"Create Socket", _T(__FUNCTION__), __LINE__); 
	return (m_hSocket != INVALID_SOCKET);
}

BOOL CNtpSocket::Connect(char* pszHostAddress, int nPort)
{
	...
	LPSTR lpszAscii = (LPSTR)pszHostAddress;
	...
	//nPort = 123;
	sockAddr.sin_port = htons((UINT)nPort);
	//sockAddr.sin_addr.s_addr = inet_addr(lpszAscii);
	sockAddr.sin_addr.s_addr = inet_addr(pszHostAddress);

	if (m_pModule != NULL)
		m_pModule->Log(PB_LOG_INFO, L"Connect Socket", _T(__FUNCTION__), __LINE__); 
...
BOOL CNtpSocket::Connect(const SOCKADDR* lpSockAddr, int nSockAddrLen)
{
	...
	if (m_pModule != NULL)
		m_pModule->Log(PB_LOG_INFO, L"Connect", _T(__FUNCTION__), __LINE__); 
	return (nConnect == 0);
}
...
BOOL CNtpSocket::Send(LPCSTR pszBuf, int nBuf)
{
	...
	//ASSERT(m_hSocket != INVALID_SOCKET);
	if (m_pModule != NULL)
		m_pModule->Log(PB_LOG_INFO,L"Send", _T(__FUNCTION__), __LINE__); 
...
int CNtpSocket::Receive(LPSTR pszBuf, int nBuf)
{
	...
	if (m_pModule != NULL)
		m_pModule->Log(PB_LOG_INFO, L"Recieve", _T(__FUNCTION__), __LINE__); 
...
CSystemTime::CSystemTime(PBModule* pModule)
{
	m_dwSntpTimeout = 5000; //Default timeout of 5 seconds
	m_pModule = pModule;
}

BOOL CSystemTime::GetSntpServerTime(char* pszHostName, NtpServerResponse& response, int nPort, BOOL modem)
{
	TCHAR cLog[256];

	//CNtpRAS* pRAS = new CNtpRAS();
	//pRAS->m_hRASConn = NULL;
	//if (pRAS->Connect(modem) != 0)
	//{
	//	Log("RAS failed to connect.\r\n");
	//	return FALSE;
	//}
	...
	CNtpSocket* pSocket = new CNtpSocket(m_pModule);
	if (!pSocket->Create())
	{
		wsprintf(cLog, L"Failed to create client socket, GetLastError returns: %d\r\n", GetLastError());
		if (m_pModule != NULL)
			m_pModule->Log(PB_LOG_WARNING, cLog, _T(__FUNCTION__), __LINE__);   
	...
	{
		wsprintf(cLog, L"Could not connect to the SNTP server %s on port %d, GetLastError returns: %d\r\n", pszHostName, nPort, GetLastError());
		if (m_pModule != NULL)
			m_pModule->Log(PB_LOG_WARNING, cLog, _T(__FUNCTION__), __LINE__);
	...
	else
	{
		...
		{
			wsprintf(cLog, L"Failed in call to send NTP request to the SNTP server, GetLastError returns %d\r\n", GetLastError());
			if (m_pModule != NULL)
				m_pModule->Log(PB_LOG_WARNING, cLog, _T(__FUNCTION__), __LINE__);
		...
		if (!pSocket->IsReadible(bReadable, m_dwSntpTimeout) || !bReadable)
		{
			wsprintf(cLog, L"Unable to wait for NTP reply from the SNTP server, GetLastError returns %d\r\n", WSAETIMEDOUT);
			if (m_pModule != NULL)
				m_pModule->Log(PB_LOG_WARNING, cLog, _T(__FUNCTION__), __LINE__);
			//Tidy up prior to returning
			delete pSocket;
		...
		{
			wsprintf(cLog, L"Unable to read reply from the SNTP server, GetLastError returns %d\r\n", GetLastError());
			if (m_pModule != NULL)
				m_pModule->Log(PB_LOG_WARNING, cLog, _T(__FUNCTION__), __LINE__);
			//Tidy up prior to returning
			DWORD dwError = GetLastError();
...
BOOL CSystemTime::EnableSetTimePriviledge()
{
...
void CSystemTime::RevertSetTimePriviledge()
{
...
{
	// Fix to get around SetSystemTime problem
	
#define MARCH 3
#define OCTOBER 10
	int Daylight = 0;	// 0 is false
	bool TimezoneFlag = false;
	...
	//wcscpy(tzi.StandardName, _T("GMT"));
	SetTimeZoneInformation(&tzi);
		

	int MarchDate = 31 - ((int) (st.wYear * 5 /4) +4) % 7;
	int OctoberDate = 31 - ((int) (st.wYear * 5 /4) +1) % 7;
		
	if ((st.wMonth > MARCH) && (st.wMonth < OCTOBER)) {
		// In BST
		Daylight = 1;
	}
	else if (st.wMonth == MARCH) {
		if (st.wDay > MarchDate) {
			// After BST date
			Daylight = 1;
		}
		else if (st.wDay == MarchDate) {
			if (st.wHour >= 1) {
	...
	else if (st.wMonth == OCTOBER) {
		if (st.wDay < OctoberDate) {
			// Still BST
			Daylight = 1;
		}
		else if (st.wDay == OctoberDate) {
			if (st.wHour < 1) {
				// We are still in british summer time
				Daylight = 1;
			}
			else if (st.wHour == 1) {
				// Special exception to the set daylight rules
				// this hour requires the ClockNot.exe to be disabled
				// because it seems to put the system time back by one hour
				TimezoneFlag = true;
			}
		}
	}
	if (TimezoneFlag) 
		CeRunAppAtEvent(_T("clocknot.exe"), 0);
	
	SetDaylightTime(Daylight);
	SetSystemTime(&st);
	
	if (TimezoneFlag) 
		CeRunAppAtEvent(_T("clocknot.exe"), NOTIFICATION_EVENT_TIME_CHANGE);
	...
	if (EnableSetTimePriviledge())
	{
		...
		if (!bSuccess)
			_tprintf(_T("Failed in call to set the system time\n"));
	}
...
BOOL CSystemTime::SetTime(LPCWSTR szTime, TimeFormat fmt)
{
	BOOL bResult = FALSE;
	LPWSTR szUtcStr = NULL;
	int iUtcLen = wcslen(szTime);
	if ((iUtcLen != 25) && (iUtcLen != 19) || ((szUtcStr = new WCHAR[iUtcLen+1]) == NULL))
	{
		m_pModule->Log(PB_LOG_INFO, L"time string length incorrect - should be 19 or 25", _T(__FUNCTION__), __LINE__);
		return FALSE;
	}

	wcscpy(szUtcStr, szTime);
	LPWSTR pUtcMarker = szUtcStr;
	SYSTEMTIME newTime;
	TIME_ZONE_INFORMATION tzd;
	ZeroMemory(&newTime, sizeof(SYSTEMTIME));
	ZeroMemory(&tzd, sizeof(TIME_ZONE_INFORMATION));
	// replace the minus signs with an alpha character so wcstol can recogise the year and month
	if ((pUtcMarker[4] == L'-') && (pUtcMarker[7] == L'-') &&
		((pUtcMarker[10] == L'T') || (pUtcMarker[10] == L't')))
	{
		pUtcMarker[4] = L'T';
		pUtcMarker[7] = L'T';
		newTime.wYear = (WORD)wcstol(pUtcMarker, &pUtcMarker, 10);
		newTime.wMonth = (WORD)wcstol(++pUtcMarker, &pUtcMarker, 10);
		newTime.wDay = (WORD)wcstol(++pUtcMarker, &pUtcMarker, 10);
	}
	if ((newTime.wYear == 0) || (newTime.wMonth == 0) || (newTime.wDay == 0))
		m_pModule->Log(PB_LOG_INFO, L"Invalid date format in time string", _T(__FUNCTION__), __LINE__);
	else
	{
		newTime.wHour = (WORD)wcstol(++pUtcMarker, &pUtcMarker, 10);
		if (*pUtcMarker == L'-')
		{
			*pUtcMarker = L'T';
			newTime.wMinute = (WORD)wcstol(++pUtcMarker, &pUtcMarker, 10);
			if (*pUtcMarker == L'-')
			{
				*pUtcMarker = L'T';
				newTime.wSecond = (WORD)wcstol(++pUtcMarker, &pUtcMarker, 10);

				// workaround to fill in day of week - convert to FILETIME and back
				FILETIME ft;
				ZeroMemory(&ft, sizeof(FILETIME));
				if (SystemTimeToFileTime(&newTime, &ft))
				{
					if (FileTimeToSystemTime(&ft, &newTime))
					{
						// we at least have a valid time, but we need to update the timezone FIRST if
						// necessary, because SetTimezoneInformation doesn't actually update the sytem
						// time, it just defines how systemtime and localtime relate to each other.
						bResult = TRUE;
						
						if (iUtcLen == 25) // if theres a timezone designator
						{
							bResult = FALSE;
							if ((*pUtcMarker == L'+') || (*pUtcMarker == L'-'))
							{
								long lBias = wcstol(pUtcMarker, &pUtcMarker, 10)*60;
								if (lBias < 0)
									lBias -= wcstol(pUtcMarker, &pUtcMarker, 10);
								else
									lBias += wcstol(pUtcMarker, &pUtcMarker, 10);
								// bias is UTC offset from Local Time in windows not offset from UTC
								lBias *= -1; // invert the bias;
								GetTimeZoneInformation(&tzd);
								if (lBias == tzd.Bias) // if the timezone isn't changing we can ignore it
									bResult = TRUE;
								else
								{
									...
									ZeroMemory(&tzd, sizeof(TIME_ZONE_INFORMATION));
									tzd.Bias = lBias;
									if (SetTimeZoneInformation(&tzd))
										bResult = TRUE;
									else 
										m_pModule->Log(PB_LOG_INFO, L"Invalid time zone information", _T(__FUNCTION__), __LINE__);
								}
							}
							else m_pModule->Log(PB_LOG_INFO, L"invalid tzd format", _T(__FUNCTION__), __LINE__);
						}
						else m_pModule->Log(PB_LOG_INFO, L"Invalid time format", _T(__FUNCTION__), __LINE__);
						...
						if (bResult)
						{
							...
							if (!bResult)
								m_pModule->Log(PB_LOG_INFO, L"error setting time", _T(__FUNCTION__), __LINE__);
						}
					}
					else m_pModule->Log(PB_LOG_INFO, L"Invalid time format", _T(__FUNCTION__), __LINE__);
				}
				else m_pModule->Log(PB_LOG_INFO, L"Missing or invalid time zome information", _T(__FUNCTION__), __LINE__);
			}
			else m_pModule->Log(PB_LOG_INFO, L"Missing seconds field", _T(__FUNCTION__), __LINE__);
		}
		else m_pModule->Log(PB_LOG_INFO, L"Missing minutes field", _T(__FUNCTION__), __LINE__);

		if (!bResult)
			m_pModule->Log(PB_LOG_INFO, L"Invalid format in time string", _T(__FUNCTION__), __LINE__);
	}
	delete [] szUtcStr;
	return bResult;
}

void CSystemTime::SetPBModule(PBModule* pModule)
{
	m_pModule = pModule;
}

void IncDate(SYSTEMTIME *st) 
{
	if (st->wHour != 23) {
		st->wHour++;
	}
	else {
		// We are at the 11pm so we need to move the day forwards
		// first check we're not at the end of a month
		switch (st->wMonth) {
			case 3:
			case 5:
			case 7:
