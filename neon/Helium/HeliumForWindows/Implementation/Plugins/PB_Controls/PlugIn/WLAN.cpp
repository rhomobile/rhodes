...
BOOL CWLAN::GetAdapterStatus (ADAPTER_STATUS *pstatus)
{
	...
	if ((*lpfnCommandFusion) (dwFusionContext, RF_SIGNAL_STRENGTH_WLAN_GET, &dwAdapterHandle, sizeof dwAdapterHandle, &rssi, sizeof rssi, NULL) != FAPI_SUCCESS)
		goto Exit;

	pstatus->nRSSI = rssi;

	//  DCC: Change for SB1 build 2.1.1.14.  Signal strength percentage changed to match that reported by fusion.
