	...
	if (dwResult == E_SCN_SUCCESS) 
	{
		...
		if (dwResult == E_SCN_SUCCESS)
		{
				...
				enabledDecoderList.Decoders.dwDecoderCount)
			{
				...
				iRetVal = wcslen(L"enabled") + 1;
				if (szPropertyValue)
					if (wcscpy_s(szPropertyValue, iRetVal, L"enabled") == EINVAL)
						iRetVal = -1;
			}
			else
			{
				//  The number of enabled and supported decoders is different
				iRetVal = wcslen(L"disabled") + 1;
				if (szPropertyValue)
					if (wcscpy_s(szPropertyValue, iRetVal, L"disabled") == EINVAL)
						iRetVal = -1;
			}
		}
	}
