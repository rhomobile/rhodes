...
*/
CSignature::CSignature(HINSTANCE hInstance,HWND hParent, int iInstanceID, PBModule* pModule, 
...
BOOL CSignature::Initialise()
{
	...
	m_hSigBox = CreateWindowEx(
				WS_EX_NOANIMATION,
				WcName,NULL,WS_POPUP | WS_VISIBLE | m_showBorder,
...
BOOL CSignature::CreateDIBitmap(LPTSTR szFilePathName)
{
	...
	//  the file out to the disk (PB / RE1 behaviour) and also for the Data URI
...
BOOL CSignature::Show()
{
	...
	if(m_hSigBox){
		...
		m_pModule->RegisterForMessage(WM_MOUSEMOVE);
...
{
	m_bHidden  = TRUE;
	m_pModule->UnRegisterForMessage(WM_MOUSEMOVE);
...
* \date		August 2006 - Initial Creation, PH
*/
void CSignature::RePaintBox(HDC hDc,BOOL bFill)
{
