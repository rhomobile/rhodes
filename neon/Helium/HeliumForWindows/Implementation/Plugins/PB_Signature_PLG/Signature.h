...
#define PEN_WIDTH	1				///<  Default width of the pen.
#define DEFAULT_FILENAME	L"Signature"	///<  Default name of the captured bitmap which will be placed in the core.  
#define MAXLEN_VECTORARR	MAXURL/2		///<  The maximum length of each vectornavigate data string
...
public:  //  Methods
	...
	*  \param pModule		Pointer to the Browser CSignatureModule class
	*						which created this signature.  Used for logging and 
	*						sending navigations back to the core.
	*/
	CSignature			(HINSTANCE hInstance,HWND hParent, int iInstanceID, PBModule* pModule,
...
private:
	...
	*/
	void		RePaintBox		(HDC hDc,BOOL bFill);
	...
	*/
	...
	TCHAR		m_tcVectorNavigateURI[MAXURL];	///<  URI Registered to receive VectorNavigate Events
	WCHAR*		m_szSignatureCaptureURI;	///<  Registered URI for Signature Capture event (blob data)
	PBModule*	m_pModule;			///<  Pointer to the CSignatureModule object owning this CSignature instance.  Used for logging and navigation.
