...
describe "IO::foreach" do
  ...
  it "yields a sequence of Strings that were separated by r" do
    lines = []
    IO::foreach(File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/readlines.txt','r') do |line|
      lines << line
    end
  ...
  it "yields a sequence of paragraphs when the separator is an empty string" do
    lines = []
    IO::foreach(File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/gets.txt', "") do |line|
      lines << line
    end
    lines.should == [
      "Voici la ligne une.\nQui \303\250 la linea due.\n\n",
