...
describe "IO#readlines when passed no arguments" do
  before(:each) do
    @io = File.open(File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/readlines.txt')
  end
...
describe "IO#readlines when passed [separator]" do
  ...
  it "returns an Array containing all paragraphs when the passed separator is an empty String" do
    File.open(File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/paragraphs.txt') do |io|
...
describe "IO.readlines when passed [file_name]" do
  before(:each) do
    @file = File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/readlines.txt'
  end
  ...
  it "raises an Errno::ENOENT error when the passed file_name does not exist" do
    non_existing_file = File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/should_not_exist.txt'
...
describe "IO#readlines when passed [file_name, separator]" do
  ...
  it "returns an Array containing all paragraphs when the passed separator is an empty String" do
    para_file = File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/fixtures/paragraphs.txt'
