...
$require_fixture_dir = (File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/../../fixtures/require')
...
describe "Kernel#require" do
  ...
  it "requires arbitrarily complex files (files with large numbers of AST nodes)" do
    lambda {require File.expand_path(File.dirname(File.join(__rhoGetCurrentDir(), __FILE__))) + '/fixtures/test'}.should_not raise_error
  end
  it "loads a .rb from an absolute path and returns true" do
    path = File.expand_path(File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/../../fixtures/require/require_spec_1.rb')
  ...
  it "allows unqualified files to contain path information (just not in the beginning)" do
    name = (File.dirname(File.join(__rhoGetCurrentDir(), __FILE__)) + '/../../fixtures')
    $LOAD_PATH << name
  ...
  it "produces File.join(__rhoGetCurrentDir(), __FILE__) as the given filename and __LINE__ as the source line number" do
    Dir.chdir($require_fixture_dir) do |dir|
