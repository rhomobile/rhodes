...
public class Barcode1 implements Runnable {
	
	private static final RhoLogger LOG = RhoLogger.RHO_STRIP_LOG ? new RhoEmptyLogger() : 
		new RhoLogger("Barcode1");

	public static RubyModule Barcode1Module;    
	...
	public void run() {
		  LOG.INFO("$$$ Register Barcode1 Ruby class");
   	  	  // register Ruby Module
          Barcode1Module = RubyAPI.defineModule("Barcode1");        
          // register Ruby method
          Barcode1Module.getSingletonClass().defineMethod("native_process_string", new RubyOneArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg0, RubyBlock block) {
				if ( arg0 instanceof RubyString )
				{
					...
					return ObjectFactory.createString(result);
					  ...
					  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Barcode1.native_process_string: wrong argument type.Should be String");
				}
			}
          });
          Barcode1Module.getSingletonClass().defineMethod("calc_summ", new RubyTwoArgMethod() {
			protected RubyValue run(RubyValue receiver, RubyValue arg1, RubyValue arg2, RubyBlock block) {
				try {
						...
						if (arg1 instanceof RubyInteger) {
							...
							if (arg1 instanceof RubySymbol) {
								...
								x = rv.toInt();
								  ...
								  throw new RubyException(RubyRuntime.ArgumentErrorClass, "in Barcode1.calc_summ: wrong argument type.Should be Fixnum or Integer");
							}
						}
					}
					...
					e.printStackTrace();
					LOG.ERROR("Barcode1.calc_summ failed with exception", e);
