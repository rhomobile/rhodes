var Rho = Rho || (function ($) {
    ...
    function commonReq(params) {
        ...
        if ("number" == typeof params.valueCallbackIndex) {
            if (params.valueCallbackIndex >= params.args.length)
                throw new Exception('Value callback should be a last passed argument!');

            valueCallback = params.args.pop();

            if ("function" != valueCallback)
                throw new Exception('Value callback should be a function!');
        }
        ...
        var result = null;
        var dfr =  new $.Deferred(function(dfr) {
            $.ajax({
        ...
        return (null != valueCallback) ? dfr : result;
    }
