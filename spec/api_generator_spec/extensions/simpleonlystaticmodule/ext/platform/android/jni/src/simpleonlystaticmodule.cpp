...
extern "C" VALUE simpleonlystaticmodule_native_process_string(const char* str) {

    JNIEnv *env = jnienv();
    jclass cls = rho_find_class(env, "com/simpleonlystaticmodule/SimpleOnlyStaticModule");
...
extern "C" int simpleonlystaticmodule_calc_summ(int x, int y) {
	return (x+y);
}
